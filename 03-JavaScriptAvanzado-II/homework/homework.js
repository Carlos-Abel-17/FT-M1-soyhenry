...
function cacheFunction(cb) {
  ...
  return function (argumento){
    // si lo quisiera poner en ternario y acortar codigo de toda la funcion
    // return cache.hasOwnProperty(argumento)?cache[argumento] : (cache[argumento] = cb[argumento]) 
    if (cache.hasOwnProperty(argumento)) {
      console.log('resultado de cache',cache[argumento]);
     return cache[argumento]
    }else{
      let numero = argumento;
      let resultado = cb(argumento)
      let devuelve = cache[numero] = resultado
      console.log('resultado calculado'+ devuelve)
      return (cache[numero] = resultado)
    }
  }
}
...
console.log(squareCache(5)); // No volverá a invocar a square, simplemente buscará en la caché cuál es el resultado de square(5) y lo retornará
console.log(squareCache(10)); // Invocará a square(10), almacenará el resultado y lo retornará
console.log(squareCache(10)); // No volverá a invocar a square, simplemente buscará en la caché cuál es el resultado de square(10) y lo retornará
